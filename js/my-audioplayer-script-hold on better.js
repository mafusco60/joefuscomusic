...
var au = document.createElement('audio');
	au.src = `music/${songs[i]}.mp3`;
	au.addEventListener('loadedmetadata', function(){
	var duration = au.duration;
	var onemin = 60;
	var min = Math.floor(duration/onemin);
	//min = Math.trunc(min)
	var sec = Math.trunc (duration % onemin);
	console.log (min + ':' + sec);
	const playlistTimes = toString(`${min} + ':' + ${sec}`);
	console.log (`${playlistTimes}`);
	playlistBtns[i].innerHTML = `<h4>${songs[i]} + ${playlistTimes})</h4>`;	
}, false);
